...
public class my {
    ...
    private void createSolarSystem() {
        ...
        createSaturn(new Point(new Double3(-20, 50, -400)), 25);

        // יצירת ירח דמוי כדור
        Point moonCenter = new Point(new Double3(0, 100, -800));
        createMoon(moonCenter, 40, rockMaterial);

        // יצירת שמש כתומה לוהטת מימין לירח
        Point sunCenter = new Point(new Double3(55, 65, -50));
        double sunRadius = 25;
        ...
        scene.background=new Color(64,64,64);
    }
    private void createStarfield() {
        ...
        for (int i = 0; i < 2000; i++) {
            ...
            scene.geometries.add(new Sphere(new Point(position), 0.1 + random.nextDouble() * 0.5)
    ...
    private void addBlackTriangles() {
        ...
        // נקודת המפגש בתחתית התמונה
        Point bottomCenter = new Point(0, -100,0 );

        // משולש שמאלי
        Point leftTop = new Point(-400, 1000, -1000);
    ...
    public void testEnhancedSolarSystem() {
        ...
        camera.setImageWriter(new ImageWriter("enhancedSolarSystem", 2000, 2000))
                .setRayTracer(new SimpleRayTracer(scene))
                // .setAntiAliasing(16) // Uncomment if your ray tracer supports anti-aliasing
                .build()
                .renderImage()
                .writeToImage();
    }
